...
class VariantSelects extends HTMLElement {
  ...
  updateVariantStatuses() {
    ...
    if (this.querySelector(`[id^="variant-radios--${this.dataset.section}"]`)) {
      ...
      //Identical for dropdown layout
      // this.selectedOptionOneVariantsDropdown = JSON.parse(this.querySelector('[type="application/json"]').textContent).filter(variant => this.querySelector('option:checked').innerText.trim() === variant.option1);
    ...
    } else if (this.querySelector(`[id^="Option-${this.dataset.section}"]`)) {
      console.log('this is running as I expect', this.dataset.section)
      this.selectedOptionOneVariantsDropdown = JSON.parse(this.querySelector('[type="application/json"]').textContent).filter(variant => this.querySelector('option:checked').innerText.trim() === variant.option1);
      //We can reuse this.inputWrappers here 👍
      this.inputWrappers = [...this.querySelectorAll('.product-form__input')];
      this.option2Select = [...this.inputWrappers[1].querySelectorAll('option')];
      this.selectedExistingOptions2 = this.selectedOptionOneVariantsDropdown.filter(option => option.available).map(a => a.option2);
      this.option2Select.forEach(option => {
        if (this.selectedExistingOptions2.includes(option.getAttribute('value'))) {
          option.innerText = option.getAttribute('value')
        } else {
          option.innerText = option.getAttribute('value') + ` - ${window.variantStrings.unavailable}`
        }
      });
      if (!this.inputWrappers[2]) return;
      this.option3Select = [...this.inputWrappers[2].querySelectorAll('option')];
      this.selectedExistingOptions3 = this.selectedOptionOneVariantsDropdown.filter(option => option.available).map( a => a.option3);
      this.option3Select.forEach(option => {
        if (this.selectedExistingOptions3.includes(option.value)) {
          option.innerText = option.getAttribute('value')
        } else {
          option.innerText = option.getAttribute('value') + ` - ${window.variantStrings.unavailable}`
        }
